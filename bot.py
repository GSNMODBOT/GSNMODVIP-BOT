...
def start(message):
    ...
    welcome = f"""
🌹 أهلاً {user_name} في البوت الذكي!
...
def ai_response(message):
    try:
        ...
        response = model.generate_content(
            f"المستخدم اسمه {user_name} وسأل: {user_input}\nرد عليه بلغة عربية فصحى أو عامية مناسبة وبشكل لطيف ومفيد."
        )
        
        reply = response.text[:4000]  # التليجرام بيسمح بـ 4096 حرف كحد أقصى
